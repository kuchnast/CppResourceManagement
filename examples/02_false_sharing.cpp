...
void oneCacheLinerThread()
{
    ...
    std::osyncstream(std::cout) << "oneCacheLinerThread() spent " << elapsed.count() << " ms\n";
    if constexpr (xy)
...
void twoCacheLinerThread()
{
    ...
    std::osyncstream(std::cout) << "twoCacheLinerThread() spent " << elapsed.count() << " ms\n";
    if constexpr (xy)
...
int main()
{
    ...
    for (auto i{0}; i != max_runs; ++i)
    {
        ...
        twoCacheLiner.x.store(0);
        twoCacheLiner.y.store(0);
    }
